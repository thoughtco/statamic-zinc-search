...
// 				break;
	...
	public function __construct($config, $model = null, $prepended_path = false) {
		...
		$this->type_name = $model && method_exists($model, 'getTypeName') ? $model->getTypeName() :
			($model && method_exists($model, 'getIndexName') ? $model->getIndexName() : 'type_name');
		$this->validation = $model && isset($model->mappingProperties) ? 'strict' : false;
		$this->key_name = $model && method_exists($model, 'getKeyName') ? $model->getKeyName() : 'id';
	}
    ...
    public function paginate($perPage = null, $columns = ['*'], $pageName = 'page', $page = null) {
        ...
        return new LengthAwarePaginator(
            collect($this->raw_results['hits']['hits']),
            $this->raw_results['hits']['total']['value'],
